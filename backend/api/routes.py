...
Appointments
@router.get("/appointments", responsemodel=List[AppointmentResponse])
async def getappointments(
    ...
    appointments = query.orderby(Appointment.appointmentdate.desc()).all()
...
async def getupcomingappointments(db: Session = Depends(getdb)):
    ...
    ).orderby(Appointment.appointmentdate, Appointment.appointmenttime).limit(10).all()
...
async def gettodayappointments(db: Session = Depends(getdb)):
    ...
    appointments = db.query(Appointment).filter(
        Appointment.appointmentdate == today,
        Appointment.status == "scheduled"
    ).orderby(Appointment.appointmenttime).all()
...
Calls
@router.get("/calls", responsemodel=List[CallResponse])
async def getcalls(skip: int = 0, limit: int = 50, db: Session = Depends(getdb)):
    """Get recent calls"""
    calls = db.query(Call).orderby(Call.starttime.desc()).offset(skip).limit(limit).all()
    return calls
...
async def getcall(callid: int, db: Session = Depends(getdb)):
    ...
    if not call:
        raise HTTPException(statuscode=404, detail="Call not found")
...
async def getemergencycalls(db: Session = Depends(getdb)):
    ...
    ).orderby(Call.starttime.desc()).limit(20).all()
...
async def getanalytics(db: Session = Depends(getdb)):
    ...
    upcomingappointments = db.query(Appointment).filter(
        Appointment.appointmentdate >= today,
        Appointment.status == "scheduled"
    ).count()
    todayappointments = db.query(Appointment).filter(
        Appointment.appointmentdate == today
    ).count()
    
    # Calls stats
    totalcalls = db.query(Call).count()
    callstoday = db.query(Call).filter(
        Call.starttime >= datetime.combine(today, datetime.min.time())
    ).count()
    callsweek = db.query(Call).filter(
        Call.starttime >= datetime.combine(weekago, datetime.min.time())
    ).count()
    emergencycalls = db.query(Call).filter(Call.emergencydetected == True).count()
    
    # Average call duration
    avgdurationresult = db.query(Call).filter(Call.duration.isnot(None)).all()
    avgduration = sum(c.duration for c in avgdurationresult) / len(avgdurationresult) if avgdurationresult else 0
    
    # Intent distribution
    intents = db.query(Call.intent, db.func.count(Call.id)).groupby(Call.intent).all()
